...
class ServerApp:
    def __init__(self, root):
        ...
        # Caixa de texto para exibir mensagens e botão para enviar
        ...
        self.button_send = tk.Button(root, text="Enviar Mensagem", command=self.broadcast_message)
        self.button_send.pack()

        self.button_exit = tk.Button(root, text="Sair", command=root.quit)
        self.button_exit.pack()
        
        # Inicializar o socket e dicionário de clientes
        self.server_socket = None
        self.clients = {}  # Armazena clientes como {'username': socket}
    ...
    def handle_client(self, client_socket):
        # Receber o nome de utilizador do cliente
        ...
        while True:
            try:
                ...
                if message.startswith("@"):  # Mensagem privada no formato "@username: mensagem"
                    target, private_msg = message.split(":", 1)
                    target_username = target[1:]
                    if target_username in self.clients:
                        self.send_private_message(private_msg, target_username, username)
                    else:
                        client_socket.send("Utilizador não encontrado.".encode())
                else:
    ...
    def send_private_message(self, message, target_username, sender_username):
        private_message = f"[MI de {sender_username}]: {message}"
        self.clients[target_username].send(private_message.encode())

    def broadcast_message(self):
        message = self.entry_message.get()
        if message:
            self.broadcast(f"Servidor: {message}")
